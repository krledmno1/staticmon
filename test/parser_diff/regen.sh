...
# (cd ../parser_oracle && dune build --release ./oracle.exe) or the
# monpoly-oracle docker image (docker/oracle.Dockerfile). Override with
# ORACLE_CMD. Writes test/parser_diff/{frames.bin, expected.txt}.
#
# Usage: regen.sh [n_random] [seed]
set -euo pipefail
here="$(cd "$(dirname "$0")" && pwd)"
N=${1:-800}; SEED=${2:-20260713}
NATIVE="$here/../parser_oracle/_build/default/oracle.exe"
...
else echo "no parser oracle (build ../parser_oracle or the monpoly-oracle image)" >&2; exit 1; fi

python3 "$here/gen_corpus.py" "$N" "$SEED" > "$here/frames.bin"
